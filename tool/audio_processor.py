...
def _check_ffmpeg():
    ...
    try:
        subprocess.run(
            ["ffmpeg", "-version"], capture_output=True, check=True, timeout=2
        )
        return True
    except (
...
class AudioProcessor:
    ...
    def stream_audio(
        self, file_path: Union[str, Path], chunk_duration_sec: float = 60.0
    ):
        """
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                librosa.get_duration(path=str(file_path))
                # Get native sample rate by loading a small sample (for duration calculation)
                waveform_sample, native_sr = librosa.load(
                    str(file_path), sr=None, duration=0.1
                )
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                waveform_np, sample_rate = librosa.load(
                    str(file_path), sr=self.target_sample_rate, mono=self.to_mono
                )
