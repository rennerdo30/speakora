...
class SeamlessTranslator:
    ...
    def detect_language(self, audio_file: Path) -> str:
        ...
        logger.info(
            f"Detected language: {detected_lang} (using model's auto-detection)"
        )
    ...
    def translate_audio(
        ...
        if self.expressive_mode and reference_audio and reference_audio.exists():
            ...
            logger.info(
                f"Expressive mode: Using reference audio from {reference_audio}"
            )
            ...
            logger.info("Expressive mode: Using input file as reference audio")
        ...
        for chunk_waveform, chunk_sample_rate in self.audio_processor.stream_audio(
            input_file, CHUNK_SIZE_SEC
        ):
            got_chunks = True
            chunk_idx += 1
            ...
            if len(batch_waveforms) >= self.config.processing.batch_size:
                ...
                logger.info(
                    f"Processed chunks {current_batch_indices[0]}-{current_batch_indices[-1]}"
                )
        ...
        if output_file is None:
            output_file = (
                ...
                / self.config.paths.translated_subdir
                / f"{input_file.stem}_translated.wav"
            )
        ...
        metadata_file = (
            Path(self.config.paths.output_dir)
            / self.config.paths.metadata_subdir
    ...
    def translate_audio_stream(
        ...
        if (
            ...
            or self._streaming_context["last_source_lang"] != source_lang
        ):
        ...
        if use_context and len(self._streaming_context["audio_buffer"]) > 0:
            ...
            context_chunks = self._streaming_context["audio_buffer"][-2:] + [
                waveform_np
            ]
        ...
        # 3. Process
        inputs = self.processor(
            audios=waveform_to_process, sampling_rate=sample_rate, return_tensors="pt"
        ).to(self.device)
        ...
        translated_text = self.processor.batch_decode(tokens, skip_special_tokens=True)[
            0
        ]
        ...
        if use_context:
            ...
            if (
                len(self._streaming_context["audio_buffer"])
                > self._streaming_context["max_buffer_size"]
            ):
            ...
            if translated_text:
                ...
                if (
                    len(self._streaming_context["text_history"])
                    > self._streaming_context["max_buffer_size"]
                ):
    ...
    def _load_reference_audio(self, reference_audio_path: Path):
        ...
        try:
            logger.info(
                f"Loading reference audio from {reference_audio_path} for expressive mode..."
            )
            ...
            with torch.no_grad():
                ...
                self._reference_audio_features = torch.tensor(
                    [1.0]
                )  # Placeholder - actual features handled by model
                self._reference_audio_path = reference_audio_path

                logger.info(
                    f"Reference audio loaded for expressive mode: {reference_audio_path}"
                )
                logger.info(
                    "Expressive mode will preserve prosody and voice characteristics during translation"
                )
    ...
    def _apply_expressive_mode(self, generate_kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._reference_audio_features is not None:
            ...
            if "temperature" in generate_kwargs:
                generate_kwargs["temperature"] = max(
                    0.7, generate_kwargs["temperature"] * 0.9
                )
