...
class Worker:
    ...
    def start(self):
        ...
        if self.worker_id == "worker-main":
            ...
            if stale_jobs:
                logger.warning(
                    f"Found {len(stale_jobs)} stale jobs. Marking them as FAILED."
                )
        ...
        while self.running:
            try:
                ...
                if checkpoint:
                    logger.info(
                        f"[{self.worker_id}] Resuming job {job.id} from position {checkpoint.audio_position}"
                    )
                ...
                try:
                    ...
                    output_file = (
                        ...
                        / self.cfg.paths.translated_subdir
                    ...
                    def progress_callback(progress: float):
                        ...
                        self.queue.update_job_status(
                            job.id, JobStatus.RUNNING, progress_percent=progress
                        )
                        ...
                        if current_time - last_checkpoint_time >= CHECKPOINT_INTERVAL:
                            try:
                                self.queue.save_checkpoint(
                                    job.id, {"progress": progress}, audio_pos
                                )
                                last_checkpoint_time = current_time
                                logger.debug(
                                    f"[{self.worker_id}] Checkpoint saved for job {job.id}"
                                )
                            except Exception as e:
                                logger.warning(
                                    f"[{self.worker_id}] Failed to save checkpoint: {e}"
                                )
                    ...
                    logger.info(
                        f"[{self.worker_id}] Job {job.id} completed in {processing_time:.2f}s"
                    )

                except KeyboardInterrupt:
                    logger.info(
                        f"[{self.worker_id}] Interrupted, pausing job {job.id}..."
                    )
                    ...
                    logger.error(
                        f"[{self.worker_id}] Error processing job {job.id}: {e}"
                    )
                    self.queue.update_job_status(
    ...
    def _signal_handler(self, signum, frame):
        ...
        if self.current_job_id:
            logger.info(
                f"[{self.worker_id}] Pausing current job {self.current_job_id}..."
            )
