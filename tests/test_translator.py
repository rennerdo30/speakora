...
def mock_translator(test_config):
    with patch("tool.translator.ModelManager") as mock_mm:
        ...
        mock_batch_encoding.items.return_value = [
            ("input_features", torch.zeros(1, 100))
        ]
        mock_batch_encoding.__getitem__.side_effect = lambda k: (
            torch.zeros(1, 100) if k == "input_features" else None
        )
        # And satisfy .to()
...
def test_translate_audio(mock_translator, tmp_path):
    ...
    with patch(
        "tool.audio_processor.AudioProcessor.stream_audio"
    ) as mock_stream, patch("tool.audio_processor.AudioProcessor.save_audio"):
...
def test_translate_audio_stream_context_reset_on_language_change(mock_translator):
    ...
    assert (
        len(translator._streaming_context["audio_buffer"]) == 1
    )  # Reset, then new chunk added
