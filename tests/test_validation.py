...
def test_cli_translate_validation(tmp_path):
    ...
    result = runner.invoke(
        cli, ["translate", "--input", str(input_file), "--target-lang", "xyz"]
    )
...
def test_cli_job_submit_validation():
    ...
    result = runner.invoke(
        cli, ["job", "submit", "--input", "test.wav", "--target-lang", "xyz"]
    )
    assert (
        result.exit_code == 0
    )  # It returns early with error message, click default exit code 0 unless exception/sys.exit
...
def test_api_validation(tmp_path):
    ...
    response = client.post(
        "/api/jobs", json={"input_file": "/tmp/audio.wav", "target_lang": "fra"}
    )
    ...
    # Invalid lang
    response = client.post(
        "/api/jobs", json={"input_file": "/tmp/audio.wav", "target_lang": "xyz"}
    )
    assert response.status_code == 400
