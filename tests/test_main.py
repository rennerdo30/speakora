...
def test_translate_command(runner, tmp_path):
    ...
    with patch("tool.main.SeamlessTranslator") as mock_translator:
        ...
        instance.translate_audio.return_value = {
            "status": "success",
            "target_file": "out.wav",
        }
        result = runner.invoke(
            cli, ["translate", "--input", str(input_file), "--target-lang", "deu"]
        )
        assert result.exit_code == 0
...
def test_gui_command(runner):
    with patch("uvicorn.run") as mock_run:
        with patch("tool.api.create_app"):
...
def test_job_submit_command(runner, tmp_path):
    ...
    with patch("tool.main.JobQueue") as mock_queue:
        ...
        result = runner.invoke(
            cli, ["job", "submit", "--input", str(input_file), "--target-lang", "deu"]
        )
        assert result.exit_code == 0
        ...
        result = runner.invoke(
            cli, ["job", "submit", "--input", str(input_dir), "--target-lang", "deu"]
        )
        assert result.exit_code == 0
        assert "Submitted 1 jobs" in result.output

        result = runner.invoke(
            cli, ["job", "submit", "--input", "invalid", "--target-lang", "deu"]
        )
